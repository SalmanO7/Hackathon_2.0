...
const CartPage = () => {
  ...
  const { setSubtotal } = useCart(); // Access setSubtotal from context

  useEffect(() => {
    localStorage.setItem("cartItems", JSON.stringify(cartItems));
    // Update subtotal in context when cartItems change
    const newSubtotal = cartItems.reduce(
      (acc: number, item: any) => acc + item.product.price * item.quantity,
      0
    );
    setSubtotal(newSubtotal); // Set subtotal in context
